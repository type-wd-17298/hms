...
class ServiceList extends \yii\db\ActiveRecord {
    ...
    public function rules() {
        return [
            ...
            [['service_list_issue', 'service_list_solve', 'service_list_comment', 'asset_list_number'], 'string'],
            ...
            [['service_list_code'], 'unique'],
                ...
                //return true;
                // return ($model->service_problem_id <> 1);
                // }],
//            ['service_list_date_finish', 'required', 'when' => function ($model) {
...
//                }],
    ...
    public function beforeSave($insert) {
        ...
        if ($this->service_status_id == 5) {
            if ($this->service_list_date_finish == '')
                $this->service_list_date_finish = new \yii\db\Expression('NOW()');
        }
        if ($this->service_status_id == 3) {
            if ($this->service_list_date_accept == '')
                $this->service_list_date_accept = new \yii\db\Expression('NOW()');
        }
