...
use app\modules\office\models\LeaveMainSearch;
use app\modules\office\models\LeaveProcessList;
use app\modules\office\models\LeaveStatus;
...
class LeavemainController extends Controller {
    ...
    public function actionIndex() {
        ...
        $searchModel = new LeaveMainSearch();
        $dataProvider = $searchModel->search(Yii::$app->request->queryParams);
        $events = [];
        foreach ($dataProvider->getModels() as $row) {
            //$Event = [];
            if ($row->leave_type_id <> '9') {
                if ($row->leave_type_time == 'F') {
                    $Event['title'] = $row->emps->employee_fullname . "({$row->emps->dep->employee_dep_label}) " . ($row->cc == 1 ? "<- ยกเลิกวันลา" : '');
                    $Event['start'] = date('Y-m-d', strtotime($row->leave_start));
                    $Event['end'] = date('Y-m-d', strtotime($row->leave_end . '+1 day'));
                    //$Event['allday'] = true;
                } elseif ($row->leave_type_time == 'H1') {
                    $Event['title'] = $row->emps->employee_fullname . "({$row->emps->dep->employee_dep_label})" . ($row->cc == 1 ? "<- ยกเลิกวันลา" : '');
                    $Event['start'] = date('Y-m-d H:i:s', strtotime($row->leave_start . " 08:30"));
                    $Event['end'] = date('Y-m-d H:i:s', strtotime($row->leave_end . " 12:00"));
                    //$Event['overlap'] = true;
                    //$Event['allday'] = true;
                } elseif ($row->leave_type_time == 'H2') {
                    $Event['title'] = $row->emps->employee_fullname . "({$row->emps->dep->employee_dep_label})" . ($row->cc == 1 ? "<- ยกเลิกวันลา" : '');
                    $Event['start'] = date('Y-m-d H:i:s', strtotime($row->leave_start . " 13:00"));
                    $Event['end'] = date('Y-m-d H:i:s', strtotime($row->leave_end . " 16:30"));
                }
                $Event['id'] = $row->leave_id;
                $Event['color'] = $row->leaveType->leave_type_color;
                $events[] = $Event;
            }
        }
    ...
    public function actionCreate() {
        ...
        $model->leave_type_time = 'F'; //Fulltime
        $model->leave_status_id = 'L00'; //เอกสารร่าง
        list($initialPreview, $initialPreviewConfig) = $this->getInitialPreview('L' . $model->leave_id);
        if ($model->load(Yii::$app->request->post()) && $model->save()) {
    ...
    public function actionUpdate($id) {
        ...
        $model->rankdate = $model->leave_start . ' - ' . $model->leave_end;
        list($initialPreview, $initialPreviewConfig) = $this->getInitialPreview('L' . $model->leave_id);
        if ($model->load(Yii::$app->request->post()) && $model->save()) {
            $this->Uploads(false, 'L' . $model->leave_id);
            \Yii::$app->getSession()->setFlash('alert', [
    ...
    public function actionDelete() {
        ...
        $model = $this->findModel($id);
        if ($model && @in_array($model->leave_status_id, ['L00', 'L08'])) {
            if ($model->delete()) {
                if ($pd = LeaveProcessList::findOne(['leave_id' => $id])) {
                    $pd->delete();
...
//$Uploads->delete();
    ...
    protected function findModel($id) {
        if (($model = LeaveMain::findOne($id)) !== null) {
            return $model;
        }
...
#$mpdf->SetFooter('<div style="text-align:right;font-size:8px;">สำนักงานสาธารณสุขจังหวัดสุพรรณบุรี วันที่พิมพ์ ' . Ccomponent::getThaiDate(date('Y-m-d'), 'L') . ' หน้า {PAGENO} / {nb}</div>');
        @$mpdf->WriteHTML($this->renderPartial($model->leaveType->leave_type_form, ['model' => @$model, 'data' => @$data]));
        $mpdf->Output();
        return $this->render($model->leaveType->leave_type_form, []);
    }
...
//return $this->redirect(['index']);
    ...
    public function actionOperate($id) {
        ...
        $model = LeaveMain::findOne($id);
        $modelProcess = LeaveProcessList::find()
                ->where(['leave_id' => $model->leave_id])
...
//ข้อมูลผลดำเนินการเป็นหัวหน้ากลุ่มงานหรือไม่
        ...
        if (($model->employee_id <> $emp->employee_id) && $head && $head->executive->employee_executive_level == 4) { //ระดับหัวหน้ากลุ่มงาน ผ่านเอกสาร
            $header = 0;
        }
...
//}
        ...
        if (!empty($modelProcess->status->leave_status_auth)) {//ให้ผู้รับผิดชอบหนังสือสามารถดำเนินการเอกสารได้
            $role = Yii::$app->authManager->getRolesByUser($profile->user_id);
            if (in_array($modelProcess->status->leave_status_auth, array_keys($role))) {
                $canVisible = 1;
            }
        }
        ...
        if ($model->employee_id == $emp->employee_id && in_array($model->leave_status_id, ['L00', 'L08'])) { //ให้ผู้ลาสามารถดำเนินการเอกสารได้
            $canVisible = 1;
        }
        ...
        if ($model->leave_status_id == 'L99') { //ระดับ'หัวหน้ากลุ่มงานการเจ้าหน้าที่
            $header = 1;
        }
        ...
        sort($arrayStatus);
        $status = LeaveStatus::find()->where(['IN', 'leave_status_id', $arrayStatus])->all();
        $query = LeaveProcessList::find()->select(['*', 'TIMEDIFF(process_acknowledge_datetime,process_create) AS paperless_tt'])->where(['leave_id' => $model->leave_id]);
        $dataProvider = new ActiveDataProvider([
            'query' => $query,
            'pagination' => [
                'pageSize' => 100
...
//-----------------------------------------------------------------------------------------------------------------------------------------------
    ...
    public function actionCreateprocess() {
        ...
        \Yii ::$app->response->format = \yii\web\Response::FORMAT_JSON;
        if (isset($param['id'])) {
            $model = new LeaveProcessList();
            $modelLeave = LeaveMain::findOne($param['id']); //ข้อมูลใบลา
//---------------------------------------------------------------------------------------------------------------------------------------------------------------------------
            $stepUp = 0;
            $stepUpHead = 0;
            $stepUpOperation = 0;
            $stepOwnerUp = 0;
            $headOwnerUp = @EmployeePositionHead::find()->where(['employee_id' => $modelLeave->employee_id])->all();
            ...
            $headOwner = @EmployeePositionHead::findOne(['employee_dep_id' => $modelLeave->emp->employee_dep_id, 'employee_id' => $modelLeave->employee_id]);
...
//---------------------------------------------------------------------------------------
            ...
            $model->leave_id = $modelLeave->leave_id; //ใบลา
            ...
            if (isset($param['cancelLeave']) && $param['cancelLeave'] == 'cancel') { //ปฏิเสธใบลาพักผ่อน
                $model->leave_status_id = 'L08'; //ปฏิเสธ
            ...
            } elseif (in_array($modelLeave->leave_status_id, ['L00', 'L08']) && $modelLeave->leave_type_id == 1) { //ลาพักผ่อน
                $model->leave_status_id = 'L01'; //
                ...
                $model->process_receiver = $modelLeave->leave_assign;
                $auth = Employee::find()->where(['IN', 'employee_id', [$model->process_receiver]])->all(); //เป็นรายบุคคล
                foreach ($auth as $emp) {
                    $linebot = new lineBot();
                    $message = " คุณถูกเลือกให้รับปฏิบัติงานแทนจาก " . $modelLeave->emps->employee_fullname;
                    @$linebot->send($message, [$emp->employee_linetoken]); //ส่ง line ให้ส่วนตัว
                }
            } elseif (in_array($modelLeave->leave_status_id, ['L00', 'L08']) && $modelLeave->leave_type_id <> 1) { //ลาอื่นๆ
                if ($stepUpHead == 1) { //รอง ผอ.
                    $model->leave_status_id = 'L99'; //งาน จ
                } else {
                    if (empty($param['receiver'])) {
...
//-------------------------------------------------------------------------------------------------
                    ...
                    if ($stepUpOperation == 1 || $stepOwnerUp == 1) { //หัวหน้างาน->เป็นหัวหน้ากลุ่มงาน ----> ระบบจะส่งไปดำเนินการในฐานนะหัวหน้ากลุ่มงานแทน
                        $model->leave_status_id = 'L03'; //
                    } else {
                        $model->leave_status_id = 'L02'; //
                    }
                    ...
                    foreach ($auth as $emp) {
                        $linebot = new lineBot();
                        $message = "คุณมีใบลาส่งมาให้ดำเนินการค่ะ " . $modelLeave->emps->employee_fullname . " " . $modelLeave->leaveType->leave_type_name . ' ระหว่างวันที่ ' . Ccomponent::getThaiDate($modelLeave->leave_start) . ' ถึง ' . Ccomponent::getThaiDate($modelLeave->leave_end);
                        @$linebot->send($message, [$emp->employee_linetoken]); //ส่ง line ให้ส่วนตัว
                    }
                }
//-------------------------------------------------------------------------------------------------------
            } elseif ($modelLeave->leave_status_id == 'L01') { //ผู้รับงานแทน/ลาพักผ่อน
            ...
            } elseif ($modelLeave->leave_status_id == 'L02') { //ส่งให้หัวหน้าฝ่ายเห็นชอบ
                $model->leave_status_id = 'L03'; //เสนอหัวหน้างาน/หัวหน้าฝ่าย
                if (empty($param['receiver'])) {
                    $message = 'ไม่พบการระบุรายชื่อผู้เสนอ กรุณาระบุชื่อผู้เสนอด้วยค่ะ';
                    return ['status' => 'error', 'message' => $message];
                }
                ...
                foreach ($auth as $emp) {
                    $linebot = new lineBot();
                    $message = "คุณมีใบลาส่งมาให้ดำเนินการค่ะ คุณ" . $modelLeave->emps->employee_fullname . " " . $modelLeave->leaveType->leave_type_name;
                    @$linebot->send($message, [$emp->employee_linetoken]); //ส่ง line ให้ส่วนตัว
                }
                //}
            } elseif ($modelLeave->leave_status_id == 'L03') { //ส่งงาน จ.
                $model->leave_status_id = 'L99'; //ส่งงาน จ.
            } elseif ($modelLeave->leave_status_id == 'L99') { //ส่งรอง ผอ.
                if ($stepUp == 0 && $stepUpHead == 0) {
                    $model->leave_status_id = 'L04'; //ส่งรอง ผอ.
                } else {
                    $model->leave_status_id = 'L05'; //ส่ง ผอ.
                }
            ...
            } elseif (in_array($modelLeave->leave_status_id, ['L04', 'L05'])) { //ส่งงาน จ.
                $model->leave_status_id = 'L10'; //ดำเนินการสำเร็จ
...
//-------------------------------------------------------------------------------------------------------
            ...
            if (!empty($model->leave_status_id)) {
                if ($model->save()) {
                    $status = 'success';
                    $message = 'ดำเนินการสำเร็จ';
                    $modelLeave->leave_status_id = $model->leave_status_id;
                    $modelLeave->leave_lastprocess_id = $model->processlist_id;
                    ...
                    if (!$modelLeave->save()) {
                        ...
                        @Cmqtt::public('hms/service/paper/update/L-' . $modelLeave->leave_assign, 'Leave-system'); //Update MQTT ผู้รับมอบ
                        @Cmqtt::public('hms/service/paper/update/L-' . $modelLeave->employee_id, 'Leave-system'); //Update MQTT เจ้าของเรื่อง
                        @Cmqtt::public('hms/service/operation/' . $model->process_receiver, date('Y-m-d H:i:s')); //Update MQTT เจ้าของเรื่อง
                        @Cmqtt::public('hms/service/operation/' . $modelLeave->leave_assign, date('Y-m-d H:i:s')); //Update MQTT เจ้าของเรื่อง
                    }
                } else {
...
//$filePath = Paperless::getUploadUrl() . $model->ref . '/thumbnail/' . $model->real_filename;
    ...
    public function actionProcessUpdate() {
        ...
        if (\Yii::$app->user->can('SuperAdmin')) {
            $model = LeaveProcessList::findOne($id);
            $lid = $model->leave_id;
            if ($model !== NULL) {
                if ($model->delete()) {
    ...
    public function actionProcessRefresh($id) {
        $model = LeaveProcessList::find()->where(['leave_id' => $id])->orderBy(['process_create' => SORT_DESC])->one();
        $modelLeave = LeaveMain::findOne($model->leave_id);
        $modelLeave->leave_status_id = $model->leave_status_id;
        $modelLeave->leave_lastprocess_id = $model->processlist_id;
        $modelLeave->update_at = new \yii\db\Expression('NOW()');
        $modelLeave->rankdate = new \yii\db\Expression('NOW()');
