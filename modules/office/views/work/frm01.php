...
<div style="text-align:left;font-size: 14pt;margin-top: 10px;" >
    <div class="room-row">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
        ...
        <?PHP IF ($model->work_change_id <> 3) { ?>
            ...
            และจะใช้คืนในวันที่<span class="dotted">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<?= Ccomponent::getThaiDate($model->work_grid_change_date_b, 'L') ?>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span>
    ...
    <table border="0" width="100%" style="font-size: 14pt;">
        <tr>
            <td width="45%" align="center"  style="vertical-align: top">
                ลงชื่อ&nbsp;&nbsp;<?PHP echo @Html::img((!empty($model->emps->employee_id) ? $model->emps->getUploadPath() : '') . '../laysen/' . @$model->emps->employee_id . '.jpg', ['width' => 80]); ?>&nbsp;&nbsp;ผู้ขอ<?= $model->workChange->work_change_name ?>
            ...
            <td width="45%" align="center" style="vertical-align: top">
                <?PHP IF (!empty($model->processL1->employee_id) && !in_array($model->work_status_id, ['L01', 'L08'])) { ?>
                    ลงชื่อ <?PHP echo @Html::img((!empty($model->processL1->employee_id) ? $model->emps->getUploadPath() : '') . '../laysen/' . @$model->processL1->receiver->employee_id . '.jpg', ['width' => 80]); ?>&nbsp;&nbsp;ผู้รับ<?= $model->workChange->work_change_name ?>
        ...
        <tr>
            <td align="center"  style="vertical-align: top">
                <?PHP IF (!empty($model->processL2->employee_id) && !in_array($model->work_status_id, ['L02', 'L08'])) { ?>
                    ลงชื่อ <?PHP echo @Html::img((!empty($model->processL2->employee_id) ? $model->emps->getUploadPath() : '') . '../laysen/' . @$model->processL2->receiver->employee_id . '.jpg', ['width' => 80]); ?>&nbsp;&nbsp;หัวหน้าหน่วยงาน
            ...
            <td align="center" style="vertical-align: top">
                <?PHP IF (!empty($model->processL3->employee_id) && !in_array($model->work_status_id, ['L01', 'L02'])) { ?>
                    ลงชื่อ <?PHP echo @Html::img((!empty($model->processL3->employee_id) ? $model->emps->getUploadPath() : '') . '../laysen/' . @$model->processL3->receiver->employee_id . '.jpg', ['width' => 80]); ?>&nbsp;&nbsp;หัวหน้ากลุ่มงาน
