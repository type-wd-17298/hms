...
echo GridView::widget([
    ...
    'columns' => [
        ...
        [
            'headerOptions' => ['class' => 'font-weight-bold'],
            'attribute' => 'paperless_official_order',
            'label' => 'สั่ง ณ วันที่',
            'format' => 'raw',
            'value' => function ($model) {
                $date = Ccomponent::getThaiDate(($model['paperless_official_order']), 'S');
                return'<span class="badge badge-rounded badge-outline-primary">' . $date . '</span>';
            }
        ],
        [
            ...
            'value' => function ($model) {
                return empty($model->paperless_official_number) ? Html::button('ออกเลขคำสั่ง', [
                    'data-id' => $model->paperless_id,
                    'data-project' => $model->paperless_id,
                    'data-gen' => 'ออกเลขหนังสือส่ง/คำสั่ง',
                    'data-number' => 0,
                    'class' => 'btnGenNum btn btn-xs btn-secondary active'
                ]) : substr($model->paperless_official_number, 8) . '/' . substr($model->paperless_official_number, 3, 4);
            }
        ],
        [
            'headerOptions' => ['class' => 'font-weight-bold'],
            ...
            'value' => function ($model) {
                ...
                return Html::a($topic . '<br>' . $model->dep->employee_dep_label, 'javascript:;', [
                    'class' => 'btnView',
                    'data' => [
                        'id' => @$model['paperless_id'],
                        'toggle' => 'tooltip',
    ...
    <div class="modal-dialog modal-xl" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h3 class="modal-title font-weight-bold">ออกเลขหนังสือราชการ (แฟ้มคำสั่ง)</h3>
