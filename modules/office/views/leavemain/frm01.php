...
<div style="text-align:center;font-size: 18pt;"><b><u>แบบใบลาพักผ่อน</u></b></div>
...
<div style="text-align:left;font-size: 16pt;margin-top: 10px;" >
    <div class="room-row">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
        ...
        <br>กลุ่มงาน/งาน <span class="dotted"><?= $model->emps->dep->employee_dep_label ?>
            &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
        </span>โรงพยาบาลสมเด็จพระสังฆราช องค์ที่ 17 สำนักงานสาธารณสุขจังหวัดสุพรรณบุรี สำนักงานปลัดกระทรวงสาธารณสุข

        <br>มีวันลาพักผ่อนสะสม <span class="dotted">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<?= @$model->emps->empLeave->cumulative ?>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span>
        วันทำการ มีสิทธิลาพักผ่อนประจำปีนี้อีก 10 วันทำการ รวมเป็น <span class="dotted">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<?= @$model->emps->empLeave->vacation_accrued ?>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span> วันทำการ
        ...
        <br>ในระหว่างลาจะติดต่อข้าพเจ้าได้ <span class="dotted">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<?= (!empty($model->leave_address) ? $model->leave_address : $model->emps->employee_address) ?>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span>
        <br>หมายเลขโทรศัพท์ <span class="dotted">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<?= $model->emps->employee_phone ?>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span>
    </div>
    <div style="text-align:right;font-size: 16pt;margin-top: 10px;">
        ขอแสดงความนับถือ&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
    ...
    <table border="0" width="100%">
        <tr>
            <td width="50%"  style="vertical-align: top">
                <div style="text-align:right;font-size: 16pt;margin-top: 30px;margin-bottom: 10px;">
                ...
                <table border="1" width="100%" style="font-size: 16pt; border: 1px; border-collapse: collapse;">
                    ...
                    <?PHP
                    $vc = @$model->accruedSS - @$model->leave_day;
                    ?>

                    <tr>
                        <td>พักผ่อน</td>
                        <td style="text-align: center;"><?= ($model->leave_type_id == 1 ? @$model->getVacationSS(1) : 0) ?></td>
                        <td style="text-align: center;"><?= ($model->leave_type_id == 1 ? @$model->leave_day : 0) ?></td>
                        <td style="text-align: center;"><?= ($model->leave_type_id == 1 ? @$model->getVacationSS(1) + @$model->leave_day : 0) ?></td>
                    </tr>
                </table>
                <?PHP IF (!empty($model->processStaff->employee_id)) { ?>
                    <div style="text-align:right;font-size: 16pt;margin-top: 20px;">
                        <br>(ลงชื่อ) <?PHP echo @Html::img((!empty($model->processStaff->employee_id) ? $model->emps->getUploadPath() : '') . '../laysen/' . @$model->processStaff->employee_id . '.jpg', ['width' => 120, 'height' => 45]); ?> ผู้ตรวจสอบ
                        <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;(<?= @$model->processStaff->emp->employee_fullname ?>)
                        <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;(ตำแหน่ง) <?= @$model->processStaff->emp->position->employee_position_name ?>
                    </div>
                    <br>
                <?PHP } ?>
                <?PHP if (!empty($model->leave_assign)) { ?>
                    <div style="text-align:right;font-size: 16pt;margin-top: 20px;">
                        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<b><u></u></b>
                        <br>ในการลาครั้งนี้ข้าพเจ้ามอบหมายการทำงานในหน้าที่
                        <br>ให้<span class="dotted">&nbsp;&nbsp;&nbsp;<?= @$model->getLeaveAssign()->employee_fullname ?> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span>เป็นผู้ดำเนินการแทน
                        <br><br>(ลงชื่อ) <?PHP echo @Html::img((!empty($model->emps->employee_id) ? $model->emps->getUploadPath() : '') . '../laysen/' . @$model->emps->employee_id . '.jpg', ['width' => 120, 'height' => 45]); ?> ผู้มอบงาน
                        <br>(ลงชื่อ)<?PHP echo in_array($model->leave_status_id, ['L01', 'L08']) ? '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;' : @Html::img((!empty($model->leaveAssign->employee_id) ? $model->emps->getUploadPath() : '') . '../laysen/' . @$model->leaveAssign->employee_id . '.jpg', ['width' => 120, 'height' => 45]); ?>ผู้รับมอบงาน
                    </div>
                <?PHP } ?>
            </td>
            <td width="50%" align="right" style="vertical-align: top">
                <div style="text-align:right;font-size: 16pt;margin-top: 30px;">
                    <?PHP IF (!empty($model->processL3->employee_id) && !in_array($model->leave_status_id, ['L01', 'L08'])) { ?>
                    ...
                    <?PHP IF (!empty($model->processExcutive->employee_id) && !in_array($model->leave_status_id, ['L01', 'L08'])) { ?>
                        <b><u>คำสั่ง</u></b>&nbsp;&nbsp;
