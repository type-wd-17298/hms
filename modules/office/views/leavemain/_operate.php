...
//$headerUpLevel = 0;
$headOwnerUp = @EmployeePositionHead::find()->where(['employee_id' => $modelProcess->employee_id])->all();
if (in_array(2, ArrayHelper::getColumn($headOwnerUp, 'executive.employee_executive_level')) && $modelProcess->leave_status_id <> 'L99') {
    $headerUpLevel = 1;
}
...
<div id="smartwizard"  class="form-wizard  sw sw-theme-default sw-justified d-none d-sm-block ">
    <ul class="nav nav-wizard">
        <li>
            <a href="#xxx1" class="nav-link  <?= (in_array($modelProcess->leave_status_id, ['L00', 'L01', 'L02', 'L03', 'L04', 'L99', 'L10']) ? 'done' : '') ?>" >&nbsp;<span>1</span>
            </a>
            ยื่นใบลา
        </li>
        <li>
            <a href="#xxx2" class="nav-link inactive <?= (in_array($modelProcess->leave_status_id, ['L03', 'L04', 'L99', 'L10']) ? 'done' : '') ?>" >&nbsp;<span>2</span></a>
            หัวหน้างานเห็นชอบ
        </li>
        <li>
            <a href="#xxx3" class="nav-link inactive <?= (in_array($modelProcess->leave_status_id, ['L99', 'L04', 'L10']) ? 'done' : '') ?>" >&nbsp;<span>3</span></a>
            หัวหน้าฝ่ายเห็นชอบ
        </li>
        <li>
            <a href="#xxx4" class="nav-link inactive <?= (in_array($modelProcess->leave_status_id, ['L10', 'L04']) ? 'done' : '') ?>" >&nbsp;<span>4</span></a>
            เจ้าหน้าที่ตรวจสอบวันลา
        </li>
        <li>
            <a href="#xxx5" class="nav-link inactive <?= (in_array($modelProcess->leave_status_id, ['L10']) ? 'done' : '') ?>">&nbsp;<span>5</span></a>
            ผู้บริหารอนุมัติ
        </li>
    </ul>
</div>
                ...
                <div class="card">
                    <div class="pt-2 m-1">
                        <div class="mb-3 row">
                            <div class="col-sm-12">
                                <div class="alert alert-light">
                                    ...
                                    <div>ประเภทการลา : <?= $modelProcess->leaveType->leave_type_name ?></div>
                                    <?PHP if ($modelProcess->leave_type_id == '001') { ?>
                                        <p>ผู้รับมอบงานแทน : <?= $modelProcess->leaveAssign->employee_fullname ?></p>
                                    <?PHP } ?>
                                    <?PHP if (in_array($modelProcess->leave_status_id, ['L00'])) { ?>
                                        <div class="h3">
                                            สถานะหนังสือ : ยื่นใบลา
                                        </div>
                                    <?PHP } else { ?>
                                        <div class="h4">
                                            สถานะหนังสือ : <br> <?= @$modelProcess->leaveStatus->leave_status_name ?>
                                            <?PHP if (in_array($modelProcess->leave_status_id, ['L08'])) { ?>
                        ...
                        if ($canVisible) {
                            ...
                            ?>
                            <?PHP if (in_array($modelProcess->leaveStatus->leave_status_id, ['L00', 'L02', 'L99', 'L08']) && $headerUpLevel == 0) { ?>
                                <div class="mb-3 row" >
...
// Register the formatting script
                            ...
                            <?PHP } elseif ($modelProcess->leaveStatus->leave_status_id <> 'L00' && 0) { ?>
                                <div class="mb-3 row">
                                    ...
                                    <div class="col-sm-6">
                                        ...
                                        foreach ($status as $row) {
                                            ...
                                            <div class="form-check ckItems">
                                                <input class="form-check-input" type="radio" name="frmStatus" value="<?= $row->leave_status_id ?>" id="<?= $row->leave_status_id ?>">
                                                <label class="form-check-label  <?= ($row->leave_status_id == 'L100' ? 'font-weight-bold text-primary' : '') ?>" for="<?= $row->leave_status_id ?>">
                                                    <?= $row->leave_status_id ?> <?= $row->leave_status_operation ?>
                                                </label>
                                            </div>
                                        <?PHP } ?>
                                    </div>
                            ...
                            <div class="mb-3 row">
                                <div class="col-sm-12">
                                    <input type="hidden" name="id" value="<?= @$modelProcess->leave_id ?>">
                                    <input type="hidden" name="cancelLeave" id="cancelLeave" >
                                    <button type="button" class="btn btn-primary btn-block font-weight-bold btn-lg" id="btnConf"><i class="fa-solid fa-check"></i> <?= @$modelProcess->leaveStatus->leave_status_operation ?></button>
                                    <?PHP if ($modelProcess->leave_status_id <> 'L00') { ?>
                ...
                <div class="default-tab1">
                    ...
                    <div class="tab-content">
                        <div class="tab-pane fade show active" id="home" role="tabpanel">
                            <div class="pt-4">
                                <div class="embed-responsive embed-responsive-16by9">
                                    <iframe  class="embed-responsive-items" src="<?= yii\helpers\Url::to(['view', 'id' => @$modelProcess->leave_id]) ?>#view=FitW" type="application/pdf" /></iframe>
                                </div>
                            </div>
                        </div>
                        <div class="tab-pane fade" id="paper" role="tabpanel">
                            <div class="pt-4">
                                <?PHP
                                $pdf = $modelProcess->getUrlPdf('L' . $modelProcess->leave_id);
                                ?>
                                <div class="embed-responsive embed-responsive-1by1">
                                    <iframe  class="embed-responsive-items" src="<?= yii\helpers\Url::to(['view', 'file' => @$pdf[0]]) ?>#view=FitH" type="application/pdf" /></iframe>
                                </div>
                            </div>
                        </div>
                                    ...
                                    [
                                        ...
                                        'value' => function ($model) {
                                            ...
                                            if (!empty($model->process_receiver)) {
                                                ...
                                                return @$model->status->leave_status_name . '<br>' . $emp;
                                            }
                                        }
                                    ],
                                    [
...
//                        }
                                    ...
                                    [
                                        ...
                                        'visible' => 1,
                                        'value' => function ($model) {
                                            return'<span class="badge badge-rounded badge-primary">' . @$model['status']['leave_status_name'] . '</span>';
                                        }
                                    ],
                                    [
